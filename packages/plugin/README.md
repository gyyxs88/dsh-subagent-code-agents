...
- **工具**：`subagent_code`、`coding_sessions_list`、`coding_session_read/start/send/cancel`、`coding_runs_list`、`coding_run_read/resume/cancel`；后台 run 默认在终态持久回报并自动唤醒拥有者
...
- **Skill**：bundle 注册 `dsh-code-agents`；`subagent_code` 默认后台，行动顾问固定使用只读、仅后台 `action-advisor` 角色，完成时由插件自动回报；不要求持续轮询 `job_output`
